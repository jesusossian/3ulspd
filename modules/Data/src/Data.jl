...
function readData(instanceFile,params)
	...
	CW = zeros(Float64, NW, NT)
	CR = zeros(Float64, NR, NT)
	...
	for w in 1:NW
		aux = sum(DW[w,:])
		for t in 1:NT
			CW[w,t] = params.capacityw*aux/NT
		end
	end
	#println("CW = ",CW)
	...
	for r in 1:NR
		...
		#println("")
		for t in 1:NT
			CR[r,t] = params.capacityr*aux/NT
		end
	end
	#println("CR = ",CR)
	...
	inst = InstanceData(NR,NT,NI,NP,NW,HCP,SCP,DP,HCW,SCW,DW,HCR,SCR,D,C,CW,CR,DeltaW,DeltamR,cumdem)
